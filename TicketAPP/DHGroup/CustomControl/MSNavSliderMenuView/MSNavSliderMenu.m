...
@implementation MSNavSliderMenu

{
    NSInteger curSelectRow;
    UIView *bottomTabLine;
    MSNavSliderMenuType menuType;
    UIScrollView *contentScrollView;
}

- (instancetype)initWithFrame:(CGRect)frame andStyleModel:(MSNavSliderMenuStyleModel *)aStyleModel andDelegate:(id<MSNavSliderMenuDelegate>)delegate  showType:(MSNavSliderMenuType)type;{
...
- (void)selectAtRow:(NSInteger)row andDelegate:(BOOL)delegate {
    ...
    if (menuType == MSNavSliderMenuTypeTitleOnly) {
        [UIView animateWithDuration:ABS(row-curSelectRow)*0.1 delay:0.f options:UIViewAnimationOptionCurveEaseInOut animations:^{
            if (styleModel.autoSuitLineViewWithdForBtnTitle) {
                float width = [MSUtil countWidthOfString:styleModel.menuTitles[row] WithHeight:100 Font:styleModel.titleLableFont];
...
- (void)initSliderTopNavView:(MSNavSliderMenuType)type {
    
    contentScrollView = [[UIScrollView alloc] initWithFrame:self.bounds];
    [self addSubview:contentScrollView];
    
    NSAssert(styleModel.menuTitles, @"顶部滑动栏标题为空?");
    if (type == MSNavSliderMenuTypeTitleAndImage) {
        if(!styleModel.menuImagesNormal) {
            NSAssert(0, @"MSNavSliderMenuTypeTitleAndImage却未传入未选中的显示图片数组?");
        }
        if (!styleModel.menuImagesSelect) {
            NSAssert(0, @"MSNavSliderMenuTypeTitleAndImage却未传入选中时的显示图片数组?");
        }
    ...
    if (!styleModel.sliderMenuTextColorForNormal) {
        styleModel.sliderMenuTextColorForNormal = MSRGB(140, 140, 140);;
    }
    ...
    if (menuType == MSNavSliderMenuTypeTitleOnly) {
        ...
        bottomTabLine=[[UIView alloc] initWithFrame:CGRectMake(styleModel.menuHorizontalSpacing, 0, styleModel.menuWidth, styleModel.lineHeight>0?styleModel.lineHeight:0)];//底部线条的高度
...
@implementation MSNavSliderMenuStyleModel
