...
#define FOREGROUND_STAR_IMAGE_NAME @"detail_comment_star_tap"
#define BACKGROUND_STAR_IMAGE_NAME @"detail_comment_star"
#define DEFALUT_STAR_NUMBER 5
#define ANIMATION_TIME_INTERVAL 0.2

@interface TYStarRateView ()
...
- (UIView *)createStarViewWithImage:(NSString *)imageName {
    ...
    float margin = (self.bounds.size.width*0.2)/4;
    for (NSInteger i = 0; i < self.numberOfStars; i ++)
    {
        
        UIImageView *imageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:imageName]];
        imageView.frame = CGRectMake(i * (self.bounds.size.width*0.8 / self.numberOfStars+margin), 0, self.bounds.size.width*0.8 / self.numberOfStars, self.bounds.size.height);
...
- (void)layoutSubviews {
    ...
    __weak TYStarRateView *weakSelf = self;
