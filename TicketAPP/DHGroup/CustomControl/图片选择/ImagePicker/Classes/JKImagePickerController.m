...
@property (nonatomic, assign) BOOL      canTap;  // 防止cell被连续点击
...
{
    if (_delegate && [_delegate respondsToSelector:@selector(imagePickerController:didSelectAssets:isSource:)]) {
...
- (UIButton *)arrowImageView{
    if (!_arrowImageView) {
        UIImage  *img = [UIImage imageNamed:@"标签-向下"];
        UIImage  *imgSelect = [UIImage imageNamed:@"标签-向上"];
...
- (UIToolbar *)toolbar{
    if (!_toolbar) {
        ...
        [_finishButton setBackgroundImage:[JKUtil stretchImage:[self imageWithColor:[UIColor blackColor]] capInsets:UIEdgeInsetsMake(5, 5, 5, 5) resizingMode:UIImageResizingModeStretch] forState:UIControlStateNormal];
        [_finishButton setBackgroundImage:[JKUtil stretchImage:[self imageWithColor:[JKUtil getColor:@"828689"]] capInsets:UIEdgeInsetsMake(5, 5, 5, 5) resizingMode:UIImageResizingModeStretch] forState:UIControlStateHighlighted];
