...
@interface XSZRadioButton()
{
    NSMutableArray* _sharedLinks;
}
@end

@implementation XSZRadioButton
...
{
    if(!_sharedLinks) {
        for(XSZRadioButton* rb in buttons) {
            if(rb->_sharedLinks) {
    ...
    BOOL (^btnExistsInList)(NSArray*, XSZRadioButton*) = ^(NSArray* list, XSZRadioButton* rb){
        ...
        if(rb->_sharedLinks!=_sharedLinks) {
            if(!rb->_sharedLinks) {
                ...
                for(NSValue* v in rb->_sharedLinks) {
                    XSZRadioButton* vrb = [v nonretainedObjectValue];
...
-(XSZRadioButton*) selectedButton
{
    if([self isSelected]) {
        return self;
    } else {
        for(NSValue* v in _sharedLinks) {
            XSZRadioButton* rb = [v nonretainedObjectValue];
            if([rb isSelected]) {
                return rb;
            }
        }
...
-(void) setSelected:(BOOL)selected distinct:(BOOL)distinct sendControlEvent:(BOOL)sendControlEvent
{
    ...
    if( distinct && (selected || [_sharedLinks count]==2) )
    {
        ...
        for(NSValue* v in _sharedLinks) {
            XSZRadioButton* rb = [v nonretainedObjectValue];
            if(rb != self) {
                [rb setButtonSelected:selected sendControlEvent:NO];
            }
        }
    }
}
