...
static NSUInteger  const kTitleTextViewCharactersCount = 15;
...
- (CGFloat)textWidth
{
   ...
   return [text sizeWithAttributes:@{NSFontAttributeName : [UIFont systemFontOfSize:14]}].width;
}
...
/** 6位数字支付密码 */
+ (BOOL)justPayPwd:(NSString *)payPwd {
    NSString *VCODE = @"^\\d{6}$";
    
    NSPredicate *regextestv = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", VCODE];
    
    if ([regextestv evaluateWithObject:payPwd] == YES) {
        return YES;
    } else {
        return NO;
    }
...
@implementation NSString (RegexOfClassMethod)
...
+ (NSDictionary *)getTodayDate
{
    ...
    NSCalendarUnit unit = kCFCalendarUnitYear|kCFCalendarUnitMonth|kCFCalendarUnitDay |kCFCalendarUnitHour;
